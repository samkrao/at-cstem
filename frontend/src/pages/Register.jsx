...
export default function Register({ onAuthed }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      const msg = e2?.response?.status === 409 ? "Email already registered." : "Register failed (backend not running?).";
  ...
  return (
    <div className="card">
      ...
      <form className="form" onSubmit={submit}>
        ...
        <div className="small">Passwords are stored securely (BCrypt) in the backend.</div>
      </form>
    </div>
  );
}
